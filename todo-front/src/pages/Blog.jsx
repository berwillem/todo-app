...
export default function Blog() {
  ...
  const handleDeletion = async () => {
    try {
      const response = await axios.delete(`http://localhost:5000/api/v1/blogs/${post._id}`)
      console.log(response)
        ...
        >
          <Box sx={style}>
            ...
            <form action="" onSubmit={createPost}>
              ...
              <textarea name="content" value={post.content} onChange={e => setPost({...post, content: e.target.value })} />
              <button >
                Submit
              </button>
          </form>
