...
exports.rateLimiter = (req, res, next) => {
    ...
    if (!userRequestLogs[userId]) {
        userRequestLogs[userId] = [];
    }

    // Filter logs to remove requests outside of the time window
    const requestLog = userRequestLogs[userId].filter(timestamp => currentTime - timestamp < rateLimitWindowMs);

    // Push the current request timestamp
    requestLog.push(currentTime);
    userRequestLogs[userId] = requestLog;
